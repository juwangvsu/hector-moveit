...
Quadrotor::Quadrotor(ros::NodeHandle& nh) : trajectory_client("/action/trajectory",true)
{
    ...
    base_sub = nh.subscribe<nav_msgs::Odometry>("/airsim_node/SimpleFlight/odom_local_ned",10,&Quadrotor::poseCallback,this);
...
void Quadrotor::findFrontier()
{
    ...
    while(!frontiers.empty())
        frontiers.pop();
    std::cout<<"requesting octomap\n";
    if(planning_scene_service.call(srv)){
	    std::cout<<"got octomap\n";
	    if (0){//for test a simple frontier to test plan
		    std::vector<std::pair<double, geometry_msgs::Pose> > candidate_frontiers;
                    geometry_msgs::Pose p;
                    p.position.x = 5;p.position.y = 1;p.position.z = 1;
                    p.orientation.w = 1;
            	    ...
            	    frontiers.push(candidate_frontiers[0]);
		    return;
	    }
        this->planning_scene->setPlanningSceneDiffMsg(srv.response.scene);
        octomap_msgs::Octomap octomap = srv.response.scene.world.octomap.octomap;
        ...
        std::vector<std::pair<double, geometry_msgs::Pose> > candidate_frontiers;
        std::cout<<"search octomap to find candidate\n";
        for(octomap::OcTree::leaf_iterator n = current_map->begin_leafs(current_map->getTreeDepth()); n != current_map->end_leafs(); ++n)
        {
            double dist = sqrt(pow(n.getX() - odometry_information.position.x,2) + pow(n.getY() - odometry_information.position.y,2) + pow(n.getZ() - odometry_information.position.z,2));
            if(!current_map->isNodeOccupied(*n) && n.getZ()>1.5 && dist < 10)//make sure the selected frontier is at least 1.5 m, to avoid blind spot in 3D map, occured at AirSimNH side walk, also select point that is less then 10 meters from current position, temprary hack...
            {
                double x_cur = n.getX();
                double y_cur = n.getY();
                double z_cur = n.getZ();
	...
	std::cout<<"candidate frontier # "<< candidate_frontiers.size();
	...
	std::cout<<"pick indices #"<<indices.size()<<std::endl;
        for(int i=0;i<indices.size();i++){
		std::cout<<i<<' ';
...
bool Quadrotor::go(geometry_msgs::Pose& target_)
{
    ...
    if (odometry_information.position.z <0){
	    std::cout<<"flip z\n";
	    odometry_information.position.z = - odometry_information.position.z;
    }
    ...
    if(this->isPathValid){
		...
		//flip the z axis for airsim, ned coord
	    trajectory[i].position.z= - trajectory[i].position.z;
            if(i==0){
...
void Quadrotor::takeoff()
{
    ...
    /* skip wait for motorenable service for airsim
